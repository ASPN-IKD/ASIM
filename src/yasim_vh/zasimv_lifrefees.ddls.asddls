...
define view entity ZASIMV_LIFREFEES as select from I_Supplier
 association [0..*] to I_SupplierPurchasingOrg        as _SupplierPurchasingOrg        on  $projection.LifreFees = _SupplierPurchasingOrg.Supplier
 association [0..*] to I_Supplier        as _Text        on  $projection.LifreFees = _Text.Supplier
 
{   
    
    @ObjectModel.text.element: ['LifreFeest']
    @EndUserText.label: '공급업체'
    key Supplier as LifreFees,
